...
int main()
{
	...
	Game->add(initGameTabs());
	Info->add(initInfoTabs());
	
	ReboundRumble->add(Game);
	ReboundRumble->add(Info);
	...
	while(true)
	{
		...
		{
			ReboundRumble->handle();
			ReboundRumble->draw();
			ReboundRumble->update();
